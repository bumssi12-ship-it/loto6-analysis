"""
app.py — ロト6統計分析 Streamlit 대시보드
GA4 태그 + Measurement Protocol 포함
"""
import os
import time
import uuid
...
def get_client_id() -> str:
    """세션별 고유 client_id를 생성/유지 (활성 사용자 집계용)."""
...
def get_session_id() -> str:
    """세션 시작 시각 기반 session_id를 생성/유지 (세션 귀속용)."""
...
def inject_ga4_tag() -> None:
    """GA4 글로벌 태그를 Streamlit에 삽입 (클라이언트 측 gtag.js)."""
    if not GA4_ID:
        return
    components.html(f"""
    <script async
...
def send_ga4_event(event_name: str, params: dict | None = None) -> None:
    """
    GA4 Measurement Protocol로 서버 측 커스텀 이벤트 전송.
    pip install ga4mp

    engagement_time_msec / session_id가 없으면 GA4가 조회수(hit)는
    기록해도 '활성 사용자'로 집계하지 못하는 문제가 있어 항상 포함한다.
    """
    if params is None:
        params = {}
    try:
...
recent_n = st.sidebar.number_input(
    "ホット/コールド判定 直近N回", min_value=10, max_value=100,
    value=30, step=5
)
lang = st.sidebar.selectbox("言語 / 언어", ["日本語", "한국어"])

# フィルタリング (r_range는 (min, max) 튜플)
